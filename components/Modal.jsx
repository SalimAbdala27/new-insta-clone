...
const Modal = () => {
  ...
  const uploadPost = async () => {
    if(loading) return; // make sure they cant upload too many times 
    setLoading(true);
    // create a post and add to firestore 'posts' collections
    // get post ID for the newly created post
    // upload the img to firebase storage with post ID
    // get a download URL from firebase storage and update the original post with img await for fb
    ...
    await uploadString(imageRef, selectedFile, "data_url").then(async snapshot => {
      ...
      await updateDoc(doc(db, 'posts', docRef.id), { // wait for the doc to update in order to post 
        image: downloadURL
      })
    })
    setOpen(false); // after the image has loaded improve user experience 
    setLoading(false);
    setSelectedFile(null);
  }
  const addImageToPost = (e) => {
    const reader = new FileReader();
    if (e.target.files[0]){ //get the file user selected
      reader.readAsDataURL(e.target.files[0]); 
    }
    reader.onload = (readerEvent) => { // once browser read the file stores as an object in the store
